  ...
  Widget build(BuildContext context) {
    return Consumer<DashboardProvider>(
      builder: (context, provider, child) {
        final trends = provider.customerTrends;
        final isLoading = provider.isCustomerLoading;
        ...
        if (trends != null && trends['labels'] != null && trends['values'] != null) {
          ...
          final values = List<dynamic>.from(trends['values']);
          spots = values.asMap().entries.map((e) => FlSpot(e.key.toDouble(), (e.value as num).toDouble())).toList();
        }
              ...
              Row(
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    height: getHeightRatio() * 36,
                    child: FilterDropDown(
                      items: const ['Last Week', 'Last Month'], // Simplified options
                      ...
                      onChanged: (value) {
                        if (value != null) {
                          ...
                          provider.fetchCustomerTrends(period);
                        }
                      },
              ...
              SizedBox(
                ...
                child: isLoading
                    ...
                    : spots.isEmpty
                        ? Center(
                            child: Column(
                              ...
                              children: [
                                Icon(Icons.show_chart, size: 48, color: AppColors.textGreyColor.withOpacity(0.5)),
                                const SizedBox(height: 8),
                                CustomText(text: "No customer data available", color: AppColors.textGreyColor),
                              ],
                            ),
                          )
                        : LineChart(
                  LineChartData(
                    ...
                    maxX: (xLabels.length - 1).toDouble(),
                    minY: 0,
                    maxY: spots.isEmpty ? 10 : (spots.map((e) => e.y).reduce(max) * 1.2),
                    gridData: const FlGridData(show: false),
                    borderData: FlBorderData(
                      show: true,
                      border: const Border(
                      ...
                      bottomTitles: AxisTitles(
                        sideTitles: SideTitles(
                          ...
                          getTitlesWidget: (value, meta) {
                            ...
                            if (xLabels.length > 7 && index % 2 != 0) return const SizedBox.shrink();
